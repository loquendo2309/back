...
// Proteger todas las rutas de orders con JWT
 ...
 *         descripcion: { type: string, example: "Switches azules, layout ES" }
 *         cantidad:    { type: number, example: 2 }
 *         precio:      { type: number, example: 50 }
 *         descuento:   { type: number, example: 5 }
 *         total:
 *           type: number
 *           description: Derivado = (precio - descuento) * cantidad (calculado en backend)
 *           example: 90
 *       required: [producto, cantidad, precio]
 *     Order:          # RESPUESTA (puede mostrar timestamps readOnly)
 *       type: object
 *       properties:
 *         _id:   { type: string, example: "675f0d2e8f2b9a001234efab" }
 *         items:
 *           type: array
 *           items: { $ref: '#/components/schemas/OrderItem' }
 *         createdAt: { type: string, format: date-time, readOnly: true }
 *         updatedAt: { type: string, format: date-time, readOnly: true }
 *     OrderInput:     # ENTRADA (sin timestamps)
 *       type: object
 *       properties:
 *         items:
 *           type: array
 ...
 *               total:
 *                 type: number
 *                 description: permitido en input pero será recalculado en backend
 *                 example: 90
 *       required: [items]
 */
 ...
 *           application/json:
 *             schema: { $ref: '#/components/schemas/Order' }
 *       400: { description: Bad request (validación) }
 */
